...
export default function ContractsPage() {
  ...
  const { data, isLoading, isError } = usePolicies({ page, size: 20, status: status || undefined });
  ...
  return (
    <div className="p-6 max-w-300 mx-auto">
      <h1 className="text-2xl font-bold text-on-surface mb-6">계약 현황</h1>
            ...
            {!isLoading &&
              ...
              data?.content.map((c, i) => (
                <tr
                  key={c.id}
                  className={
                    i < (data.content.length - 1) ? "border-b border-outline-variant" : ""
                  }
                >
